...
def parens_match_iterative(mylist):
    ...
    """
    ### TODO
    current_output = iterate(parens_update, 0, mylist)
...
def parens_match_scan(mylist):
    ...
    """
    ###TODO
    res = scan(lambda x,y: x+y if x >= 0 else -float("inf"), 0, list(map(paren_map,mylist)))
...
def parens_match_dc_helper(mylist):
    ...
    leftright, leftleft = parens_match_dc_helper(mylist[0:len(mylist)//2])
    rightright, rightleft = parens_match_dc_helper(mylist[len(mylist)//2:])

    if leftleft < 0:
        return (0,-float('inf'))
    return (leftright - rightleft, leftleft - rightright)
